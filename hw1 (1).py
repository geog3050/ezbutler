...
def import_data(filename):
    ...
    with open(filename) as roster:
        read_roster= csv.reader(roster, delimiter=',')
        for x in read_roster:
            Pokemon=[str(x[0]),str(x[1]),float(x[2]),float(x[3])]
...
def fight(participant1, participant2, first2attack):
    ...
    HP1= participant1[2]
    HP2= participant2[2]
    switch_attacker= lambda attacker: 2 if attacker==1 else 1
    while HP1 > 0 and HP2 > 0:
        if first2attack==1:
            multi= attack_multiplier(participant1[1],participant2[1])
            HP2-= (multi*participant1[3])
        elif first2attack==2:
            multi= attack_multiplier(participant2[1],participant1[1])
            HP1-= (multi*participant2[3])
        rounds+=1 #what constitutes a round??
        first2attack= switch_attacker(first2attack)
    if HP1>0:
        winner  = 1
    else:
        winner = 2
...
def tournament(participants):
    ...
    for x in participants:
        ...
        while opp_index < len(participants):
            home= fight(x,participants[opp_index],1)
            away= fight(x,participants[opp_index],2)
            if (home[0]+away[0])==2:
                wins[(participants.index(x))]+=2
            elif(home[0]+ away[0])==3:
                wins[(participants.index(x))]+=1
                wins[opp_index]+=1
            else:
                wins[opp_index]+=2
            opp_index+=1
