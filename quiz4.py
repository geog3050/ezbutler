...
######################################################################
    mode=input('Please Enter Your Desired Method:')
    count=int(input('Please Enter How Many Values You Wish TO Analyize'))
    values=[]
...
######################################################################
    if mode=='min':
        ...
        inter=1
        total=values[0]
        while inter<len(values):
            total+=values[inter]
            inter+=1
        ans=total/(len(values))
        print(ans)
